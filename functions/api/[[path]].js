...
export async function onRequest(context) {
  ...
  if (segs[0] === "bootstrap" && method === "GET") {
    ...
    const settings = { params: {}, prices: {}, specs: {}, materials: {}, formulas: {}, cparams: [], templates: [] };
  ...
  if (segs[0] === "settings") {
    if (segs.length === 1 && method === "GET") {
      ...
      const out = { params: {}, prices: {}, specs: {}, materials: {}, formulas: {}, cparams: [], templates: [] };
      for (const row of results) { try { out[row.k] = JSON.parse(row.v); } catch (e) {} }
      return json(out);
    }
    if (segs.length === 2 && method === "PUT") {
      const key = segs[1];
      if (!["params", "prices", "specs", "materials", "formulas", "cparams", "templates"].includes(key)) return bad("未知设置键");
